...
import greedySDCH.Model.{Id, StringStatisticSet}

import scala.collection.mutable.ListBuffer

object LongestCommonSubstring extends WellKnownSubstring {
  def longestCommonSubstring(values : StringStatisticSet, leftBorder : Int = 1, rightBorder : Int = Int.MaxValue) : Option[String] = {
    var left : Int = leftBorder
    var right : Int = Math.min(rightBorder, values.values.map(x => x._1.length).max)

    while (left < right) {
      val mid = (right + left + 1) / 2
      val intermediateResult: Option[String] = find(values, mid)
  ...
  def find(values : StringStatisticSet, length : Int) : Option[String] = {
    val context = newContext()
    for (struct <- values.values) {
      hash(struct, length, context)
    }

    val best: (ListBuffer[(Id, Int)], Int) = context.getBestPositions
